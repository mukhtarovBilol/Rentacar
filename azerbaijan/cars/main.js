...
function getModelFromTitle() {
    ...
    if (titleElement) {
        ...
        // Удаляем "Rent" в начале и "icarəsi" в конце
        const modelText = titleText.replace(/^Rent\s+/, '').replace(/\s*icarəsi\s*$/, '');
        ...
        if (data.hasOwnProperty(key)) {            
            if (key === model) {
                ...
                prices.forEach((priceInfo, index) => {
                    ...
                    priceLabel.textContent = priceInfo.azDays;
...
function sendMail() {
    if (document.querySelector("#name").value !== '' && document.querySelector("#message").value !== '') {
        ...
        if (checkbox?.classList == 'checkbox active') {
            var childrenSitting = "Bəli"
        } else {
            childrenSitting = "yox"
        }

        var params = {
            name: document.querySelector("#name").value + ' ' + "Avtomobil istehsalı " + getTitle + ", Avtomobilin alınma tarixi və vaxtı: " + getcar + ' ' + time + ', Qaytarma tarixi və vaxtı: ' + comeback + ' ' + endTime,
            message: countryCode.value + phoneNumber + ', Gmail müştəri ' + getEmail + ', Mənə avtomobil oturacağı lazımdırmı?: ' + childrenSitting + ', Yeri götürün: ' + getsCars + ', Avtomobilin qaytarılma yeri: ' + backCars + ', Sərnişinlərin sayı: ' + countPassanger + ", Cəmi: " + morePrice?.innerHTML
        };
        ...
        emailjs.send(serviceID, templateID, params)
            .then(res => {
                alert("təşəkkür edirəm. Ərizə qəbul edilib. Menecerimiz qısa zamanda sizinlə əlaqə saxlayacaq.")
                document.getElementById("my-modal").classList.remove("open")
