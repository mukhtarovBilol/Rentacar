// Удаление rent из слова и вывод цен
...
function displayCarPrices(data, modelName, container) {
...
async function updateCarPricing() {
    ...
    document.querySelectorAll('.cars__card').forEach(card => {
        ...
        if (container) {
            displayCarPrices(data, modelName, container);
        }
    });
}

// Вызываем основную функцию при загрузке страницы
document.addEventListener('DOMContentLoaded', updateCarPricing);